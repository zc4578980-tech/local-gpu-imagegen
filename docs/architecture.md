...
| `WorkflowOnboarding` | Inspects one ordinary `txt2img` API workflow, infers a single checkpoint or split model binding, computes `source_sha256` and `workflow_sha256`, and orchestrates confirmed immutable registration | MCP transport, trust mutation, UI-format conversion, or backend startup |
...
10. `local_gpu_recommend_models` applies hard capability filters and returns one route plus at most two alternatives. A regional request also binds normalized copy/subject geometry to the reviewed `sdxl-regional-txt2img` capability. The Agent displays the exact route and obtains a new post-display confirmation.
11. `AssetRunEngine` consumes the single-use `route_token`, persists endpoint/model/workflow/compiler identity and any regional geometry, and rechecks them before backend work.
12. Revision services copy the reviewed parent source or validate a confirmed mask; a child inherits the parent route and cannot override it.
13. The exact WebUI, ComfyUI, or Diffusers compatibility adapter returns one normalized result. Only a validated PNG and matching identity consume a successful round.
14. Structured data is returned as MCP `structuredContent` plus text content; tool failures use `isError: true`, while protocol failures use JSON-RPC error envelopes.
...
Safe workflow onboarding is separate from model trust. It accepts one bare API graph or one unique `prompt` wrapper for ordinary `txt2img` only, supports single checkpoint and split model topologies, and rejects UI format, custom nodes, regional onboarding, two-stage onboarding, img2img, and inpainting. `source_sha256` binds the exact input bytes; `workflow_sha256` binds the canonical registered semantics. A stale source, changed proposal, ambiguous loader, diagnostic inventory, invalid confirmation, or registered-copy drift fails before mutation. Registration returns a `registered_workflow_id`; later trust consumes that ID with fresh exact component identities.
