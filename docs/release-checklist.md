...
- [ ] A fresh isolated Python 3.12 environment installs the final exact-commit wheel outside the checkout and verifies version `0.8.1`, protocol `2024-11-05`, and exactly seventeen tools; exact-commit Python 3.11 verification remains a CI gate.
...
- [ ] README, changelog, package metadata, plugin metadata, `server.json`, GitHub copy, directory copy, and release notes agree on version `0.8.1`, exactly seventeen tools, three backends, the ordinary golden path, and open limitations.
...
- [ ] The official MCP Registry record resolves version `0.8.1` to the exact PyPI package and stdio command.
- [ ] Tag `v0.8.1`, the preview release, package URL, Registry URL, and repository evidence URLs resolve to the exact verified release state.
