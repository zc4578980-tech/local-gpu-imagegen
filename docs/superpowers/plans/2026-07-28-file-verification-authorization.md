...
Expected: record the registry's exact net production increase and subtract it from the slice-wide 350-line ceiling; `git diff --check` emits no output. Do not trim schema checks to create room for later integration.
...
Expected: record the cumulative new production net lines and the exact remainder under the 350-line ceiling. If the remaining lines cannot fit the already specified runtime and MCP edits, stop before Task 3 for design review.
...
Expected: the four production owners total at most 350 net new lines for this slice. Stop before commit if the ceiling is exceeded; tests and docs do not count toward that production ceiling.
