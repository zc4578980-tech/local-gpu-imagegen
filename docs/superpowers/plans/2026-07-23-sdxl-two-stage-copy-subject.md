...
`validate_saved_soft_mask` must require equal RGB channels, zero RGB outside the subject rectangle, a positive strict interior, and no nonzero pixel in the protected copy rectangle. Preserve the approved installed `FeatherMask` loop order exactly: left, right, top, then bottom. The right and bottom loops index `-distance`; because `-0 == 0`, their first iterations attenuate the first column and first row a second time instead of selecting the opposite edge. Later right/bottom iterations start at the last column/row with factor `2 / f`. `SaveImage` truncates the resulting float32 `255 * value` to 8-bit RGB, so at `f = 32` the first subject row and column quantize to zero while ordinary right and bottom outer-edge values begin at `15`. Validate this asymmetric quantized zero/positive envelope plus nondecreasing inward direction across every subject row on the left/right edges and every subject column on the top/bottom edges. Require actual profiles to rise or stay flat exactly where the installed envelope does. `f = 1` is a valid unchanged hard edge because every applied multiplier is `1/1`; for `f = 0`, accept a positive hard perimeter with a positive strict interior.
