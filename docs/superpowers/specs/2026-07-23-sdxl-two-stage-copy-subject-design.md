...
The approved `FeatherMask` semantics are also bound to the installed built-in implementation. It applies the left, right, top, and bottom loops in that order. The right and bottom loops index `-distance`; because `-0 == 0`, each loop's first iteration attenuates the first column or first row again, while its next iteration selects the opposite outer edge with factor `2 / f`. `SaveImage` then clips the float32 `255 * value` and truncates it to unsigned 8-bit RGB. With `f = 32`, the repeated attenuation makes the first subject row and column quantize to `0`, while ordinary right and bottom outer-edge values begin at `15`. These asymmetric quantized zeros are expected runtime output and must not be rejected as an invalid ramp.
