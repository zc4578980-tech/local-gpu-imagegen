...
Safe onboarding accepts ordinary `txt2img` API workflows only: single checkpoint or split model. `unsupported_workflow_envelope` often means UI format; enable ComfyUI developer mode and export API format. `unsupported_workflow_operation` or `unsupported_workflow_topology` means img2img, inpaint, regional/two-stage onboarding, custom nodes, or mixed loaders are outside scope. `ambiguous_workflow_binding` means the graph has multiple plausible samplers, prompts, latent sources, outputs, or component paths; use a simpler API workflow or the legacy explicit binding path.

`diagnostic` inspection means the graph could be inferred but current inventory did not bind every component exactly. Run API-only discovery and inspect again. `workflow_proposal_stale` means source bytes, semantic graph, proposal digest, or inventory changed after inspection; display the new `source_sha256` and `workflow_sha256`, then obtain a new later confirmation. `invalid_workflow_confirmation` means the exact `register_workflow:<source_sha256>:<proposal_digest>` value was missing or changed. `workflow_registration_drifted` means the immutable registered copy no longer matches its digest; do not repair it in place. Registration does not grant model trust, so use `registered_workflow_id` only in a separate `local_gpu_set_model_trust` flow with exact component identities.
