...
`sdxl-checkpoint-source-audit.json` records the separately approved checkpoint transfer and read-only inspection. Its status is `downloaded_untrusted`: the exact file size and SHA-256 match the mirror's repository-linked response, the safetensors header declares StabilityAI and CreativeML Open RAIL++-M, and ComfyUI exposes the checkpoint through `CheckpointLoaderSimple`. The audit grants no trust or public authority; an SDXL-specific reviewed workflow, confirmation-gated binding, and acceptance authority are still required.
