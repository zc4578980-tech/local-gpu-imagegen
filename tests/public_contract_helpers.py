...
_CLAIM_RULES = tuple(
    ...
    for category, pattern in (
        ...
        ("performance", r"\b(?P<predicate>supports?|handles?)\s+\d+\s+concurrent\s+(?:generations?|requests?|jobs?)\b"),
        ("quality", r"\bimage\s+quality\b.{0,32}?\b(?:is\s+)?(?P<predicate>better|best|superior)\b"),
        ("popularity", r"\b(?P<predicate>guaranteed|guarantees?)\b.{0,48}?\b\d+\s+stars?\b"),
